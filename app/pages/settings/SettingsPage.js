...
import {StyleSheet, View, DeviceEventEmitter, StatusBar, Dimensions, Platform, Linking } from 'react-native'
import {Container, Header, Left, Button, Icon, Right, Card, CardItem, Text, Content } from 'native-base'
import {SinglePickerMaterialDialog} from 'react-native-material-dialog'
import I18n from "../../lang/i18n"
import {EsWallet, D} from 'esecubit-wallet-sdk'
import {version} from '../../../package.json'
import {LEGAL_CURRENCY_UNIT_KEY, ETH_UNIT_KEY, BTC_UNIT_KEY, RESULT_OK, MOCK_URL} from '../../common/Constants'
...
export default class SettingsPage extends Component {
  ...
  _getCurrencyPreference() {
    PreferenceUtil.getCurrencyUnit(LEGAL_CURRENCY_UNIT_KEY).then(value => this.setState({legalCurrencyLabel: value}))
    PreferenceUtil.getCryptoCurrencyUnit(ETH_UNIT_KEY).then(value => this.setState({ethLabel: value}))
    PreferenceUtil.getCryptoCurrencyUnit(BTC_UNIT_KEY).then(value => this.setState({btcLabel: value}))
  }
      ...
      <Container style={[CommonStyle.layoutBottom, {backgroundColor: Color.CONTAINER_BG}]}>
        <Header style={{backgroundColor: '#1D1D1D'}}>
          ...
          <View style={platform === 'ios' ? CommonStyle.toolbarIOS : CommonStyle.toolbarAndroid}>
            <Text style={{color: Color.ACCENT, fontSize: Dimen.PRIMARY_TEXT, marginBottom:platform === "ios" ? 15 : 0}}>{I18n.t('settings')}</Text>
          </View>
          <Right />
        </Header>
        <Content style={{backgroundColor: Color.CONTAINER_BG}}>
          <Card style={{flex: 1}}>
            ...
            <CardItem bordered button onPress={() => {
              this.isConnected ? (ToastUtil.showShort(I18n.t('hasConnected'))) : (_that.props.navigation.navigate('PairList', {hasBackBtn: true}))
            }}>
            ...
            <CardItem bordered button onPress={() => this.setState({disConnectDialogVisible: true})}>
            ...
            <CardItem bordered button onPress={() => _that.setState({legalCurrencyDialogVisible: true})}>
              <Text>{I18n.t('legalCurrency')}</Text>
              <Right>
                <View style={{flexDirection: 'row'}}>
                  <Text style={{marginRight: Dimen.MARGIN_HORIZONTAL}}>{this.state.legalCurrencyLabel}</Text>
                  <Icon name='ios-arrow-forward'/>
                </View>
              </Right>
            </CardItem>
            ...
            <CardItem bordered button onPress={() => this.setState({btcDialogVisible: true})}>
              ...
              <Right>
                <View style={{flexDirection: 'row'}}>
                  <Text style={{marginRight: Dimen.MARGIN_HORIZONTAL}}>{this.state.btcLabel}</Text>
                  <Icon name='ios-arrow-forward'/>
                </View>
              </Right>
            </CardItem>
            ...
            <CardItem bordered button onPress={() => this.setState({ethDialogVisible: true})}>
              ...
              <Right>
                <View style={{flexDirection: 'row'}}>
                  <Text style={{marginRight: Dimen.MARGIN_HORIZONTAL}}>{this.state.ethLabel}</Text>
                  <Icon name='ios-arrow-forward'/>
                </View>
              </Right>
            </CardItem>
        ...
        <SinglePickerMaterialDialog
          ...
          items={Object.values(D.unit.legal).map((row, index) => ({value: index, label: row}))}
          colorAccent={Color.ACCENT}
          okLabel={I18n.t('confirm')}
          cancelLabel={I18n.t('cancel')}
          visible={this.state.legalCurrencyDialogVisible}
          selectedItem={{value: this.state.legalCurrencyIndex, label: this.state.legalCurrencyLabel}}
          ...
          onOk={result => {
            ...
            this.setState({legalCurrencyDialogVisible: false, legalCurrencyLabel: label, legalCurrencyIndex: index})
        ...
        <SinglePickerMaterialDialog
          ...
          selectedItem={{value: this.state.btcIndex, label: this.state.btcLabel}}
          onCancel={() => this.setState({btcDialogVisible: false})}
          onOk={result => {
            let label = result.selectedItem.label
            let index = result.selectedItem.value
            this._updateCurrencyPreference(BTC_UNIT_KEY, label, index)
            this.setState({btcDialogVisible: false, btcLabel: label, btcIndex: index})
        ...
        <SinglePickerMaterialDialog
          ...
          selectedItem={{value: this.state.ethIndex, label: this.state.ethLabel}}
          onCancel={() => this.setState({ethDialogVisible: false})}
          onOk={result => {
            let label = result.selectedItem.label
            let index = result.selectedItem.value
            this._updateCurrencyPreference(ETH_UNIT_KEY, label, index)
            this.setState({ethDialogVisible: false, ethLabel: label, ethIndex: index})
            console.log('emit', label)
        ...
        <Dialog.Container visible={this.state.disConnectDialogVisible} style={{marginHorizontal: Dimen.MARGIN_HORIZONTAL}}>
          ...
          <Dialog.Button style={{color: Color.ACCENT}} label={I18n.t('cancel')} onPress={() => this.setState({disConnectDialogVisible: false})}/>
          <Dialog.Button style={{color: Color.ACCENT}} label={I18n.t('confirm')} onPress={() => this._disConnect()}/>
        </Dialog.Container>
      </Container>
    )
  }
