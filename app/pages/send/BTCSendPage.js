...
export default class BTCSendPage extends React.Component {
  ...
  _fillResendData() {
    if (this.txInfo !== undefined) {
      let value = this.txInfo.outputs.find(output => !output.isMine) === undefined ? this.txInfo.outputs[0].value : this.txInfo.outputs.find(output => !output.isMine).value
   ...
   */
  _checkFormData() {
    return this._checkAddress(this.state.address.trim())
  ...
  _getBTCMaxAmount(formData) {
    this.account.prepareTx(formData)
      .then(result=>{
        ...
        let value = this.esWallet.convertValue(this.coinType, result.outputs[0].value, this.minimumUnit, this.cryptoCurrencyUnit)
  ...
  _toMinimumUnit(value) {
    return this.esWallet.convertValue(this.coinType, value, this.cryptoCurrencyUnit, D.unit.btc.satoshi)
  }
  ...
  render() {
    return (
      <Container style={CommonStyle.layoutBottom}>
              ...
              <Item>
                <Text style={[CommonStyle.secondaryText, {marginRight: Dimen.SPACE}]}>{I18n.t('fee')}</Text>
                {
                  this.state.currentFeeType === 'standard'
                    ? <Dropdown
                      containerStyle={{flex: 1, marginLeft: Dimen.SPACE, paddingBottom: 12}}
                      data={this.state.feesTip}
                      fontSize={14}
                      itemTextStyle={{textAlign: 'center', flex: 0}}
                      value={this.state.selectedFeeTip}
                      onChangeText={(value, index, data)=> {this._calculateBTCFee(this.state.fees[index].toString())}} />
                    : <Input
                      selectionColor={Color.ACCENT}
            ...
            <CardItem>
              ...
              <View style={{flex: 1, marginLeft: 32}}>
                ...
                <View style={{flexDirection: 'row', justifyContent:'flex-end', marginTop: Dimen.SPACE}}>
                  <Text style={{color: Color.LIGHT_PARIMARY, fontSize: Dimen.SECONDARY_TEXT}}>
                    {this.state.totalCostLegalCurrency + ' '}
                  </Text>
                  <Text style={{color: Color.LIGHT_PARIMARY, fontSize: Dimen.SECONDARY_TEXT, textAlignVertical: 'center'}}>
        ...
        <Dialog.Container visible={this.state.deviceLimitDialogVisible} style={{marginHorizontal: Dimen.MARGIN_HORIZONTAL}}>
