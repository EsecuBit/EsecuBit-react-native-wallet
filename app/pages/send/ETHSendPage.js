...
import {View, StyleSheet, Platform, DeviceEventEmitter,TouchableOpacity} from 'react-native'
import I18n from '../../lang/i18n'
import {Dropdown} from 'react-native-material-dropdown'
import {Container, Content, Icon, Text, Button, Footer, FooterTab, Card, CardItem, Item, Input} from 'native-base'
...
export default class ETHSendPage extends React.Component {
  ...
  _fillResendData() {
    if (this.txInfo !== undefined) {
      ...
      this.setState({
        ...
        sendValue: this.esWallet.convertValue(this.coinType, value, D.unit.eth.Wei, this.cryptoCurrencyUnit),
  ...
  componentDidMount() {
    this._initListener()
    this._getSuggestedFee()
      .catch(err => {
        console.warn('getSuggestedFee error', err)
        ToastUtil.showErrorMsgLong(err)
      })
    let balance = this.esWallet.convertValue(this.coinType, this.account.balance, this.minimumUnit, this.cryptoCurrencyUnit)
  ...
  _initListener() {
    ...
    this.esWallet.listenTxInfo(async () => {
      ...
      if (this.state.remarks !== undefined && this.state.remarks !== '') {
        ...
        this.account.updateTxComment(txInfo)
  ...
  _convertToSuggestedFeeTip(coinType, fees) {
    ...
    for (let i = 0; i < feeLevel; i++) {
      ...
      let feeValue = this.esWallet.convertValue(coinType, feeValues[i], D.unit.eth.Wei, D.unit.eth.GWei) + D.unit.eth.GWei
   ...
   */
  _checkFormData() {
    return this._checkAddress(this.state.address.toString())
  ...
  async _changeFeeType() {
    if (this.state.currentFeeType === 'standard') {
      ...
      await this.setState({currentFeeType: 'standard', selectedFee: this.state.fees[0]})
    }
  ...
  _getETHMaxAmount(formData) {
    this.account.prepareTx(formData)
      .then(result => {
        console.log('_maxAmount result', result)
        let fromUnit = D.isBtc(this.coinType) ? D.unit.btc.satoshi : D.unit.eth.Wei
        let value = this.esWallet.convertValue(this.coinType, result.output.value, fromUnit, this.cryptoCurrencyUnit)
  ...
  _calculateTotalCost() {
    ...
    this.account.prepareTx(formData)
      .then(value => {
        ...
        let fromUnit = D.isBtc(this.coinType) ? D.unit.btc.satoshi : D.unit.eth.Wei
        let legalCurrencyResult = this.esWallet.convertValue(this.coinType, value.total, fromUnit, this.legalCurrencyUnit)
        let cryptoCurrencyResult = this.esWallet.convertValue(this.coinType, value.total, fromUnit, this.cryptoCurrencyUnit)
        let transactionFee = this.esWallet.convertValue(this.coinType, value.fee, fromUnit, this.cryptoCurrencyUnit)
        legalCurrencyResult = StringUtil.formatLegalCurrency(Number(legalCurrencyResult).toFixed(2))
        this.setState({totalCostLegalCurrency: legalCurrencyResult, totalCostCryptoCurrency: cryptoCurrencyResult, transactionFee: transactionFee})
      ...
      <Container>
        ...
        <Content padder>
          ...
          <Card style={{marginLeft:0,marginRight:0}}>
            ...
            <CardItem>
              <Item inlineLabel>
                ...
                <Input
                  ...
                  returnKeyType='done'
                  onChangeText={(text) => this._calculateSendValue(text).catch(err => console.log(err))}
                  keyboardType={platform === "ios" ? 'numbers-and-punctuation' : 'numeric'}
                  blurOnSubmit={true}
                />
                <TouchableOpacity  small style={{marginLeft: Dimen.MARGIN_HORIZONTAL, alignSelf: 'auto'}} onPress={() => this._maxAmount(this)}>
                  <Text style={{color: Color.ACCENT, fontSize: Dimen.PRIMARY_TEXT}}>Max</Text>
                </TouchableOpacity>
              </Item>
            </CardItem>
            <CardItem>
              <Item>
                <Text style={[CommonStyle.secondaryText, {marginRight: Dimen.SPACE}]}>GasPrice</Text>
                {
                  this.state.currentFeeType === 'standard'
                    ? <Dropdown
                      ...
                      fontSize={14}
                      onChangeText={(value, index, data) => this._calculateETHFee(this.state.fees[index], this.state.gasLimit)}/>
                    : <Input
                      selectionColor={Color.ACCENT}
                ...
                <TouchableOpacity  small style={{marginLeft: Dimen.SPACE, alignSelf: 'auto'}} onPress={this._changeFeeType.bind(this)}>
            ...
            <CardItem>
              <Item >
                <Text style={[CommonStyle.secondaryText, {marginRight: Dimen.SPACE}]}>GasLimit</Text>
                <Input
                  selectionColor={Color.ACCENT}
            ...
            <CardItem>
              <Text style={[CommonStyle.secondaryText, {marginRight: Dimen.SPACE}]}>{I18n.t('transactionFee')}</Text>
            ...
            <CardItem>
              <Text style={[CommonStyle.secondaryText, {marginRight: Dimen.SPACE}]}>{I18n.t('totalCost')}</Text>
              <View style={{flex: 1, marginLeft: 32}}>
                <View style={{flexDirection: 'row', justifyContent:'flex-end'}}>
                  <Text>
                ...
                <View style={{flexDirection: 'row', justifyContent:'flex-end', marginTop: Dimen.SPACE}}>
                  <Text style={{color: '#DED1A4', fontSize: Dimen.SECONDARY_TEXT}}>
                    {this.state.totalCostLegalCurrency + ' '}
                  </Text>
                  <Text style={{color: '#DED1A4', fontSize: Dimen.SECONDARY_TEXT, textAlignVertical: 'center'}}>
